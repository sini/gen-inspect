# ══ ORIGIN ══════════════════════════════════════════════════════════════════════════════════════
# COPIED, not imported, from `gen-scope/examples/sql-schema/lib/sql.nix` at gen-scope
# `675d9f3542320546351605ba29f7c6361fcb0268`; md5 of the source file at that rev is
# `794e0c3603296cd27a6f1d673e5d41cd`.
#
# WHY A COPY. `gen-scope/examples/` is not published surface, and taking gen-scope as a second root
# input to reach one example is what ADR-0037 forbids. The design records the alternative and its
# reason: "gen-scope as an input, to reach the parser — ADR-0037. Copy with an origin header."
#
# THE WHOLE DIFF AGAINST THE ORIGIN IS THIS HEADER PLUS TWO ACCESSOR PATHS.
#   `lib.lists.findFirstIndex` → `lib.findFirstIndex`
#   `lib.strings.toInt`        → `lib.toInt`
# The `lib` this file takes is gen-prelude extended by `./extras.nix`, NOT nixpkgs `lib` — every gen
# roster member's `lib/` is nixpkgs-lib-free (ADR-0014) — and the prelude publishes a FLAT surface,
# so those two nested nixpkgs paths are the only names that could not survive the substitution
# unchanged. Nothing else moved: the tokenizer, the classifier and the recursive-descent parser are
# byte-identical to the origin.
#
# STALENESS. Nothing here re-derives this copy from its origin; the design sequences that enforcer
# to gate 2's guarantee row (§3b, "a staleness enforcer for the copied parser"). Until it exists, a
# reader comparing the two files has this header's rev and md5 to compare against.
# ════════════════════════════════════════════════════════════════════════════════════════════════
#
...
let
  ...
  toUpperChar =
    ...
    let
      ...
      idx = lib.findFirstIndex (x: x == c) null (lib.stringToCharacters lower);
    in
